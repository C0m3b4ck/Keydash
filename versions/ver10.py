...
def save_score(wpm, accuracy, time_between_letters, is_cheating, sentence):
    ...
    if is_cheating:
        # Create a cheat hash using WPM, accuracy, timestamp
        cheat_string = f"{wpm:.2f}{accuracy:.2f}{timestamp}"
        cheat_hash = hashlib.sha256(cheat_string.encode('utf-8')).hexdigest()
        cheat_content = (f"CHEAT DETECTED\nHash: {cheat_hash}\n"
                         "This session's stats are invalid due to detected macro or automated input.\n"
                         f"Sentence: {sentence}\n")

        # Write cheat hash instead of normal stats
        with open(score_filename, "w") as f:
            f.write(cheat_content)
        print(f"Cheating detected! Invalid stats saved to {score_filename}")

        # Append invalid marker to cumulative stats.txt
        with open(stats_filename, "a") as sf:
            sf.write(f"{timestamp}, CHEAT DETECTED, WPM: 0.00, Time: 0.00s, Accuracy: 0.00%, "
                     f"AvgTimeBetweenLetters: 0.000s, Sentence: {sentence}\n")
    else:
        # Save normal stats to individual file WITH SENTENCE
        with open(score_filename, "w") as f:
        ...
        # Append normal stats to cumulative stats.txt
        ...
        with open(stats_filename, "a") as sf:
            sf.write(f"{timestamp}, WPM: {wpm:.2f}, Time: {elapsed_global:.2f}s, Accuracy: {accuracy:.2f}%, "
                     f"AvgTimeBetweenLetters: {avg_time:.3f}s, Sentence: {sentence}\n")
...
def plot_stats():
    ...
    with open(stats_filename, "r") as sf:
        for line in sf:
            ...
            if len(parts) < 6:
                continue  # Skip lines that don't have expected columns
            # Skip cheat detected lines
    ...
    while True:
        ...
        while current_index < len(text):
            ...
            if ch != expected_char:
                ...
                # Wait here until correct letter is pressed, do not move ahead
                continue
        ...
        save_score(wpm, accuracy, time_stamps, is_cheating, text)
...
def main_menu():
    while True:
        print("=== Offline KeyDash Main Menu ===")
        print("1. Typing test (choose sentence)")
        print("2. View overall stats graph")
        print("3. Exit")
